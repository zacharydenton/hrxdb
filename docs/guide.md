...
and output buffers. Add `hrx = { package = "hrx-rs", version = "0.5" }` to
...
uses the published `hrx-rs 0.5` dependency and needs no sibling checkout. HRX provisions its pinned runtime automatically; use
